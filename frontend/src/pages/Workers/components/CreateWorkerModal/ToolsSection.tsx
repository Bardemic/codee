...
export function ToolsSection({ integrations, selectedTools, setSelectedTools }: ToolsSectionProps) {
    ...
    const toggleTool = (toolSlug: string) => {
        ...
        setSelectedTools((prev) => (isSelected ? prev.filter((slug) => slug !== toolSlug) : [...prev, toolSlug]));
    };
    ...
    return (
        <div className={styles.toolsWrapper}>
            <div className={styles.toolsHeader}>
                Tools <span className={styles.toolHeaderSub}>({selectedTools.length} selected)</span>
            </div>
                            ...
                            {isExpanded && (
                                <div className={styles.toolList}>
                                    {integration.tools.map((tool) => (
                                        <div key={tool.id} className={styles.toolItem} onClick={() => toggleTool(tool.slug_name)}>
