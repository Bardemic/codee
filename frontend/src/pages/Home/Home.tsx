    ...
    return (
        <div className={styles.homeContainer}>
            ...
            <div className={styles.chatContainer}>
                <textarea onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey && userMessage.length > 0 && !isCreatingWorkspace) { e.preventDefault(); createNewWorkspace(); } }} value={userMessage} onChange={(e) => setUserMessage(e.target.value)} className={styles.newWorkspace} placeholder='Find all errors from the recent commit and fix them' name="prompt" id="6-7" />
                {userMessage.length > 0 && (
                    <button className={styles.sendButton} onClick={createNewWorkspace} disabled={isCreatingWorkspace}>
